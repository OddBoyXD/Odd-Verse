...
class MusicServices extends getx.GetxService {
  ...
  Future<Response> _sendRequest(String action, Map<dynamic, dynamic> data,
    ...
    try {
      final effectiveHeaders = Map<String, String>.from(_headers);
      final payload = Map<dynamic, dynamic>.from(data);

      if (getx.Get.isRegistered<YtmAuthService>()) {
        final ytmAuth = getx.Get.find<YtmAuthService>();
        if (ytmAuth.isLoggedIn.value) {
          final authHeaders = ytmAuth.getAuthHeaders();
          if (authHeaders.isNotEmpty) {
            effectiveHeaders.addAll(authHeaders);
          }
          if (payload['context'] is Map) {
            payload['context']['user'] = {'lockedSafetyMode': false};
          }
        }
      }
  ...
  Future<Map<String, dynamic>> search(String query,
    ...
    for (var res in results) {
      ...
      if (res['itemSectionRenderer'] != null || res['musicShelfRenderer'] != null) {
        ...
        if (filter == null) {
          List<dynamic> itemResults = res['itemSectionRenderer'] != null
              ? res['itemSectionRenderer']['contents']
              : res['musicShelfRenderer']['contents'];

          if (itemResults.isNotEmpty &&
              itemResults[0]['musicResponsiveListItemRenderer'] == null) continue;
          ...
          for (var item in mixedItems) {
            ...
            if (item is MediaItem) {
              final resType = item.extras?['resultType'];
              if (resType == 'video') {
                itemType = "Videos";
              } else {
                itemType = "Songs";
