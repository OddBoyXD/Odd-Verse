...
Future<AudioHandler> initAudioService() async {
  return await AudioService.init(
    ...
    config: const AudioServiceConfig(
      ...
      androidNotificationChannelName: 'Odd Verse Notification',
...
class MyAudioHandler extends BaseAudioHandler with GetxServiceMixin {
  ...
  final Set<String> _prebufferingSongIds = {};
  ...
  MyAudioHandler() {
    if (GetPlatform.isWindows || GetPlatform.isLinux) {
      JustAudioMediaKit.title = 'Odd Verse';
    ...
    _player = AudioPlayer(
      ...
      bufferForPlaybackDuration: Duration(milliseconds: 150),
      bufferForPlaybackAfterRebufferDuration: Duration(milliseconds: 1000),
      targetBufferBytes: 5 * 1024 * 1024,
      prioritizeTimeOverSizeThresholds: true,
      backBufferDuration: Duration(seconds: 15),
    )));
    _createCacheDir();
    _addEmptyList();
  ...
  void _prebufferUpcomingSongs() {
    try {
      final appPrefs = Hive.box("AppPrefs");
      final int prebufferCount = appPrefs.get("prebufferSongCount") ?? 2;
      if (prebufferCount <= 0) return;

      final currentQueue = queue.value;
      if (currentQueue.isEmpty) return;

      List<String> songIdsToPrebuffer = [];

      if (shuffleModeEnabled && shuffledQueue.isNotEmpty) {
        final start = currentShuffleIndex + 1;
        final end = min(start + prebufferCount, shuffledQueue.length);
        if (start < shuffledQueue.length) {
          songIdsToPrebuffer = shuffledQueue.sublist(start, end);
        }
      } else if (currentIndex != null) {
        final start = (currentIndex as int) + 1;
        final end = min(start + prebufferCount, currentQueue.length);
        if (start < currentQueue.length) {
          songIdsToPrebuffer =
              currentQueue.sublist(start, end).map((e) => e.id).toList();
        }
      }

      for (final songId in songIdsToPrebuffer) {
        if (_prebufferingSongIds.contains(songId)) continue;
        ...
        if (songsUrlCacheBox.containsKey(songId)) {
          ...
          if (streamInfoJson.runtimeType.toString().contains("Map") &&
              !isExpired(
                  url: (streamInfoJson['lowQualityAudio']?['url'] ?? ''))) {
            continue;
          }
        }

        _prebufferingSongIds.add(songId);
        printINFO("Prebuffering stream URL for upcoming track: $songId");
        checkNGetUrl(songId).then((_) {
          _prebufferingSongIds.remove(songId);
        }).catchError((err) {
          _prebufferingSongIds.remove(songId);
          printERROR("Prebuffer error for $songId: $err");
        });
      }
    } catch (e) {
      printERROR("Error during upcoming songs prebuffering: $e");
    }
  }
