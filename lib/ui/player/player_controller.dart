...
class PlayerController extends GetxController
  ...
  List<SponsorSegment> _currentSponsorSegments = [];
  final Set<String> _skippedSegmentKeys = {};
  final sponsorIntroOffset = Duration.zero.obs;
  final isDismissingMiniPlayer = false.obs;
  ...
  void _listenForChangesInPosition() {
    AudioService.position.listen((position) {
      ...
      // SponsorBlock Auto-Skip check (intro & sponsor only)
      if (Get.find<SettingsScreenController>().enableSponsorBlock.isTrue &&
          _currentSponsorSegments.isNotEmpty) {
        final posSec = position.inMilliseconds / 1000.0;
        for (final seg in _currentSponsorSegments) {
          final key = "${seg.uuid}_${seg.start}_${seg.end}";
          // If user sought backwards past the segment start, reset skipped state
          if (posSec < seg.start - 1.0 && _skippedSegmentKeys.contains(key)) {
            _skippedSegmentKeys.remove(key);
          } else if (posSec >= seg.start &&
              posSec < seg.end - 0.25 &&
              !_skippedSegmentKeys.contains(key)) {
            _skippedSegmentKeys.add(key);
            printINFO(
                "SponsorBlock: Skipping ${seg.category} from ${seg.start}s to ${seg.end}s");
            final targetMs = (seg.end * 1000).toInt() + 150;
  ...
  void _listenForChangesInDuration() {
    _audioHandler.mediaItem.listen((mediaItem) async {
      ...
      if (mediaItem != null) {
        ...
        _currentSponsorSegments = [];
        _skippedSegmentKeys.clear();
        sponsorIntroOffset.value = Duration.zero;
        final songId = mediaItem.id;
        SponsorBlockService.getSegments(songId).then((segments) {
          if (currentSong.value?.id == songId) {
            ...
            if (totalIntroSec > 0.0) {
              sponsorIntroOffset.value =
                  Duration(milliseconds: (totalIntroSec * 1000).round());
              printINFO(
                  "SponsorBlock: Lyrics auto-offset +${totalIntroSec.toStringAsFixed(3)}s applied for $songId");
            }
          }
        });
        await _checkFav();
        ...
        if (isRadioModeOn && (currentSong.value!.id == currentQueue.last.id)) {
          ...
          _checkAndTriggerInfiniteRadio();
        }
        lyrics.value = {"synced": "", "plainLyrics": ""};
        if (showLyricsflag.isTrue) {
  ...
  Future<void> pushSongToQueue(MediaItem? mediaItem,
    ...
    final session = ++_playbackSessionToken;
    isDismissingMiniPlayer.value = false;
    ...
    // Start playback immediately for 0-lag instant response
    if (playlistid == null &&
    ...
    Future.delayed(
      ...
      () async {
        ...
        if (_playbackSessionToken != session) return;
  ...
  bool _isInfiniteRadioFetching = false;

  Future<void> _checkAndTriggerInfiniteRadio() async {
    final settingsCon = Get.find<SettingsScreenController>();
    if (!settingsCon.enableAutoInfiniteRadio.value) return;
    if (isLoopModeEnabled.isTrue || isQueueLoopModeEnabled.isTrue) return;
    if (_isInfiniteRadioFetching) return;
    if (currentQueue.isEmpty || currentSong.value == null) return;

    final currIndex = currentSongIndex.value;
    final isNearEnd = currIndex >= currentQueue.length - 2;
    if (!isNearEnd) return;

    _isInfiniteRadioFetching = true;
    try {
      final lastSong = currentQueue.last;
      printINFO(
          "Safe Infinite Radio: Fetching similar tracks for ${lastSong.title} (${lastSong.id})...");
      final content = await _musicServices.getWatchPlaylist(
        videoId: lastSong.id,
        radio: true,
        limit: 20,
      ...
      if (tracks.isNotEmpty) {
        ...
        printINFO(
            "Safe Infinite Radio: Appended ${tracks.length} tracks to queue.");
      }
    } catch (e) {
      printERROR("Safe Infinite Radio error: $e");
    } finally {
      _isInfiniteRadioFetching = false;
    }
  }
  ...
  Future<void> dismissMiniPlayer() async {
    ...
    isDismissingMiniPlayer.value = true;
    pause();
    try {
      seek(Duration.zero);
    } catch (_) {}
    ...
    try {
      ...
      printERROR("Failed to clear prevSessionData on dismissMiniPlayer: $e");
    }
    await Future.delayed(const Duration(milliseconds: 150));
    playerPanelMinHeight.value = 0.0;
    isPlayerpanelTopVisible.value = false;
    playerPaneOpacity.value = 0.0;
    initFlagForPlayer = true;
    isDismissingMiniPlayer.value = false;
  }
  ...
  Future<void> showLyrics() async {
    ...
    if (showLyricsflag.isTrue) {
      if (lyrics["synced"].toString().isEmpty &&
          lyrics['plainLyrics'].toString().isEmpty) {
  ...
  Future<void> fetchLyricsForCurrentSong() async {
    ...
    try {
      ...
      if (currentSong.value?.id != requestedId) return;

      if (lyricsR != null) {
        lyrics.value = lyricsR;
        if (lyricsR["synced"] == null ||
            lyricsR["synced"].toString().trim().isEmpty) {
          lyricsMode.value = 1;
        }
      } else {
