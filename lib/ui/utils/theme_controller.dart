...
class ThemeController extends GetxController {
  ...
  void setTheme(ImageProvider imageProvider, String songId) async {
    ...
    try {
      final paletteBox = Hive.isBoxOpen('paletteCache')
          ? Hive.box('paletteCache')
          : await Hive.openBox('paletteCache');
      if (paletteBox.containsKey(songId)) {
        final cached = paletteBox.get(songId);
        if (cached is Map) {
          primaryColor.value = Color(cached['primary']);
          textColor.value = Color(cached['text']);
          ...
          Hive.box('appPrefs')
              .put("themePrimaryColor", (primaryColor.value!).value);
          setWindowsTitleBarColor(themedata.value!.scaffoldBackgroundColor);
          return;
        }
      }
      ...
      PaletteGenerator generator = await PaletteGenerator.fromImageProvider(
          ResizeImage(imageProvider, height: 180, width: 180));
      ...
      if (paletteColor != null) {
        ...
        paletteBox.put(songId, {
          'primary': (primaryColor.value!).value,
